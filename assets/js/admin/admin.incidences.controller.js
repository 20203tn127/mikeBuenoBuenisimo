    ...
    fetch("http://206.189.234.55:3001/api/incidences/pending/2")
        ...
        .then((data) => {
            data.incidences.forEach((incidence, index) => {
                ...
                actions.innerHTML = `
                <button class="btn btn-success btn-edit" data-id="${incidence.id}">Aeptar</button>
                <button class="btn btn-danger btn-delete" data-id="${incidence.id}">Rechazar</button>
                `;
                ...
                btnEdit.addEventListener("click", () => {
                    alert("Aceptar incidencia con ID " + incidence.id);
                });

                btnDelete.addEventListener("click", () => {
                    alert("echazar incidencia con ID " + incidence.id);
                });
