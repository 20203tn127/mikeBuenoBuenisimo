const updateDynamicCache = (dynamicCache, req, res) => {
  ...
  const updateStaticCache = async (staticCache, req, res, APP_SHELL_INMUTABLE) => {
  ...
  const apiSaveIncidence = (cacheName, req ) => {
    if(req.url.indexOf('/api/notification/')>= 0 || req.url.indexOf('/api/notification/susbcribe')>=0) {
      return fetch(req)
